...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        run_action.triggered.connect(self.on_run_analysis_dialog)
        self.menu_analyze.addAction(run_action)
    ...
    def setup_statusbar(self):
        """Creates SAP2000-style status bar with Units Dropdown."""
    ...
    def handle_right_click(self):
        ...
        if hasattr(self.model, 'has_results') and self.model.has_results:
            ...
            if clicked_node_id is not None:
                res_action = menu.addAction(f"Joint {clicked_node_id} Displacements...")
                res_action.triggered.connect(lambda: self.show_node_results(clicked_node_id))
                menu.addSeparator()
                hit_something = True
    ...
    def delete_current_selection(self):
        ...
        self.status.showMessage("Selection Deleted.")
    ...
    def start_analysis_sequence(self, case_name):
        ...
        self.worker = SolverWorker(self.solver_input_path, self.solver_output_path)
...
def main():
    ...
    possible_paths = [
        os.path.join(root_dir, "graphic", "loading.jpg"),
        os.path.join(root_dir, "graphic", "loading.png"),
        os.path.join(current_dir, "loading.jpg")
    ]
    ...
    if splash_path:
        ...
        target_w = int(screen.geometry().width() * 0.60)
        scaled_pixmap = pixmap.scaledToWidth(target_w, Qt.TransformationMode.SmoothTransformation)
        
        splash = OPENCIVILSplash(scaled_pixmap)
        splash.show()
        
        milestones = [
            (0, "Initializing Core Engine..."),
            (15, "Loading Material Database..."),
            (30, "Initializing OpenGL Graphics..."),
            (45, "Loading Finite Element Solvers..."),
            (60, "Configuring Sparse Matrix Libraries..."),
            (75, "Loading UI Components..."),
        ...
        for i in range(101):
            if current_idx < len(milestones) - 1:
                if i >= milestones[current_idx + 1][0]:
                    current_idx += 1
            
            msg = milestones[current_idx][1]
            splash.progress(i, msg)
